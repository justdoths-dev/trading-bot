...
LOAD_BEARING_FIELDS = [
    ...
    "comparison_edge_candidate_strength_latest",
    "comparison_edge_candidate_strength_cumulative",
    ...
    "analyzer_triplet_present",
    "analyzer_selected_candidate_strength",
    "analyzer_selected_stability_label",
    "analyzer_selected_visible_horizons",
    "analyzer_preview_symbol_visible_horizons",
    "analyzer_preview_strategy_visible_horizons",
    "analyzer_sample_count",
    "analyzer_positive_rate_pct",
    "analyzer_robustness_signal_pct",
    "mapper_triplet_present",
    "mapper_triplet_visible_horizons",
    "mapper_triplet_stability_label",
    "mapper_triplet_candidate_strength",
    "mapper_triplet_sample_count",
    "mapper_triplet_positive_rate_pct",
    "mapper_triplet_robustness_signal_pct",
...
class MapperRuleRow:
    ...
    comparison_edge_candidate_strength_latest: Optional[str]
    comparison_edge_candidate_strength_cumulative: Optional[str]
    ...
    analyzer_triplet_present: bool
    analyzer_selected_candidate_strength: Optional[str]
    analyzer_selected_stability_label: Optional[str]
    analyzer_selected_visible_horizons: List[str]
    analyzer_preview_symbol_visible_horizons: List[str]
    analyzer_preview_strategy_visible_horizons: List[str]
    analyzer_sample_count: Optional[int]
    analyzer_positive_rate_pct: Optional[float]
    analyzer_robustness_signal_pct: Optional[float]

    mapper_triplet_present: bool
    mapper_triplet_visible_horizons: List[str]
    mapper_triplet_stability_label: Optional[str]
    mapper_triplet_candidate_strength: Optional[str]
    mapper_triplet_sample_count: Optional[int]
    mapper_triplet_positive_rate_pct: Optional[float]
    mapper_triplet_robustness_signal_pct: Optional[float]
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "comparison_edge_candidate_strength_latest": self.comparison_edge_candidate_strength_latest,
            "comparison_edge_candidate_strength_cumulative": self.comparison_edge_candidate_strength_cumulative,
            ...
            "analyzer_triplet_present": self.analyzer_triplet_present,
            "analyzer_selected_candidate_strength": self.analyzer_selected_candidate_strength,
            "analyzer_selected_stability_label": self.analyzer_selected_stability_label,
            "analyzer_selected_visible_horizons": self.analyzer_selected_visible_horizons,
            "analyzer_preview_symbol_visible_horizons": self.analyzer_preview_symbol_visible_horizons,
            "analyzer_preview_strategy_visible_horizons": self.analyzer_preview_strategy_visible_horizons,
            "analyzer_sample_count": self.analyzer_sample_count,
            "analyzer_positive_rate_pct": self.analyzer_positive_rate_pct,
            "analyzer_robustness_signal_pct": self.analyzer_robustness_signal_pct,
            "mapper_triplet_present": self.mapper_triplet_present,
            "mapper_triplet_visible_horizons": self.mapper_triplet_visible_horizons,
            "mapper_triplet_stability_label": self.mapper_triplet_stability_label,
            "mapper_triplet_candidate_strength": self.mapper_triplet_candidate_strength,
            "mapper_triplet_sample_count": self.mapper_triplet_sample_count,
            "mapper_triplet_positive_rate_pct": self.mapper_triplet_positive_rate_pct,
            "mapper_triplet_robustness_signal_pct": self.mapper_triplet_robustness_signal_pct,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Diagnose mapper triplet rule inputs by comparing windows where a tracked "
            "symbol/strategy/horizon triplet is emitted versus omitted."
        )
    )
...
def main() -> None:
    ...
    json_path = output_dir / f"{safe_symbol}_{safe_strategy}_{safe_horizon}_mapper_triplet_rule_diagnosis.json"
    md_path = output_dir / f"{safe_symbol}_{safe_strategy}_{safe_horizon}_mapper_triplet_rule_diagnosis.md"
    ...
    print(json.dumps(
        {
            ...
            "summary": {
                ...
                "analyzer_present_count": summary["analyzer_present_count"],
                "mapper_present_count": summary["mapper_present_count"],
            },
        },
        indent=2,
        ensure_ascii=False,
    ...
    if args.write_latest_copy:
        ...
        latest_json = DEFAULT_LATEST_DIR / f"{safe_symbol}_{safe_strategy}_{safe_horizon}_mapper_triplet_rule_diagnosis.json"
        latest_md = DEFAULT_LATEST_DIR / f"{safe_symbol}_{safe_strategy}_{safe_horizon}_mapper_triplet_rule_diagnosis.md"
...
def extract_edge_candidates_preview_horizon_block(raw_payload: Dict[str, Any], tracked_horizon: str) -> Dict[str, Any]:
    return get_nested_dict(raw_payload, "analyzer_output", "edge_candidates_preview", "by_horizon", tracked_horizon)
...
def build_mapper_rule_row(
    ...
    preview_horizon_block = extract_edge_candidates_preview_horizon_block(raw_payload, tracked_horizon)
    ...
    edge_candidates_comparison_block = get_nested_dict(comparison_report, "edge_candidates_comparison", tracked_horizon)
    ...
    comparison_edge_candidate_strength_latest = edge_candidates_comparison_block.get("latest_candidate_strength")
    comparison_edge_candidate_strength_cumulative = edge_candidates_comparison_block.get("cumulative_candidate_strength")
    ...
    analyzer_selected_candidate_strength = coalesce(
    ...
    analyzer_selected_stability_label = coalesce(
        analyzer_triplet_row.get("selected_stability_label"),
        recursive_find_first(analyzer_triplet_row, "selected_stability_label"),
    )
    analyzer_selected_visible_horizons = normalize_string_list(coalesce(
        analyzer_triplet_row.get("selected_visible_horizons"),
        recursive_find_first(analyzer_triplet_row, "selected_visible_horizons"),
    ))
    analyzer_preview_symbol_visible_horizons = normalize_string_list(coalesce(
        analyzer_triplet_row.get("preview_symbol_visible_horizons"),
        recursive_find_first(analyzer_triplet_row, "preview_symbol_visible_horizons"),
        get_nested_dict(preview_horizon_block, "top_symbol").get("visible_horizons"),
        get_nested_dict(preview_horizon_block, "top_symbol").get("selected_visible_horizons"),
    ))
    analyzer_preview_strategy_visible_horizons = normalize_string_list(coalesce(
        analyzer_triplet_row.get("preview_strategy_visible_horizons"),
        recursive_find_first(analyzer_triplet_row, "preview_strategy_visible_horizons"),
        get_nested_dict(preview_horizon_block, "top_strategy").get("visible_horizons"),
        get_nested_dict(preview_horizon_block, "top_strategy").get("selected_visible_horizons"),
    ))
    analyzer_sample_count = to_int(coalesce(
        analyzer_triplet_row.get("sample_count"),
        recursive_find_first(analyzer_triplet_row, "sample_count"),
    ))
    analyzer_positive_rate_pct = to_float(coalesce(
        analyzer_triplet_row.get("positive_rate_pct"),
        recursive_find_first(analyzer_triplet_row, "positive_rate_pct"),
    ))
    analyzer_robustness_signal_pct = to_float(coalesce(
        analyzer_triplet_row.get("robustness_signal_pct"),
        recursive_find_first(analyzer_triplet_row, "robustness_signal_pct"),
    ))

    mapper_triplet_visible_horizons = normalize_string_list(coalesce(
    ...
    mapper_triplet_stability_label = coalesce(
    ...
    mapper_triplet_candidate_strength = coalesce(
    ...
    return MapperRuleRow(
        ...
        comparison_edge_candidate_strength_latest=str(comparison_edge_candidate_strength_latest) if comparison_edge_candidate_strength_latest is not None else None,
        comparison_edge_candidate_strength_cumulative=str(comparison_edge_candidate_strength_cumulative) if comparison_edge_candidate_strength_cumulative is not None else None,
        ...
        analyzer_triplet_present=bool(analyzer_triplet_row),
        analyzer_selected_candidate_strength=str(analyzer_selected_candidate_strength) if analyzer_selected_candidate_strength is not None else None,
        analyzer_selected_stability_label=str(analyzer_selected_stability_label) if analyzer_selected_stability_label is not None else None,
        analyzer_selected_visible_horizons=analyzer_selected_visible_horizons,
        analyzer_preview_symbol_visible_horizons=analyzer_preview_symbol_visible_horizons,
        analyzer_preview_strategy_visible_horizons=analyzer_preview_strategy_visible_horizons,
        analyzer_sample_count=analyzer_sample_count,
        analyzer_positive_rate_pct=analyzer_positive_rate_pct,
        analyzer_robustness_signal_pct=analyzer_robustness_signal_pct,
        mapper_triplet_present=bool(mapper_triplet_candidate),
        mapper_triplet_visible_horizons=mapper_triplet_visible_horizons,
        mapper_triplet_stability_label=str(mapper_triplet_stability_label) if mapper_triplet_stability_label is not None else None,
        mapper_triplet_candidate_strength=str(mapper_triplet_candidate_strength) if mapper_triplet_candidate_strength is not None else None,
        mapper_triplet_sample_count=mapper_triplet_sample_count,
        mapper_triplet_positive_rate_pct=mapper_triplet_positive_rate_pct,
        mapper_triplet_robustness_signal_pct=mapper_triplet_robustness_signal_pct,
...
def build_summary(
    rows: List[MapperRuleRow],
    tracked_symbol: str,
    tracked_strategy: str,
    tracked_horizon: str,
) -> Dict[str, Any]:
    analyzer_present_rows = [row for row in rows if row.analyzer_triplet_present]
    mapper_present_rows = [row for row in rows if row.mapper_triplet_present]
    missing_rows = [row for row in rows if not row.mapper_triplet_present]
    ...
    return {
        ...
        "tracked_horizon": tracked_horizon,
        "row_count": len(rows),
        "analyzer_present_count": len(analyzer_present_rows),
        "mapper_present_count": len(mapper_present_rows),
        "mapper_missing_count": len(missing_rows),
        "present_windows": [row.end_record_index_inclusive for row in mapper_present_rows],
        "missing_windows": [row.end_record_index_inclusive for row in missing_rows],
        "analyzer_present_windows": [row.end_record_index_inclusive for row in analyzer_present_rows],
    }


def build_pairwise_comparisons(rows: List[MapperRuleRow]) -> List[Dict[str, Any]]:
    present_rows = [row for row in rows if row.mapper_triplet_present]
    missing_rows = [row for row in rows if not row.mapper_triplet_present]
    ...
    for missing_row in missing_rows:
        ...
        present_dict = nearest_present.to_dict()
        missing_dict = missing_row.to_dict()
...
def build_explanations(summary: Dict[str, Any], pairwise_comparisons: List[Dict[str, Any]]) -> List[str]:
    ...
    explanations.append(
        "This report compares mapper upstream rule inputs between windows where the tracked triplet is emitted and windows where it is omitted."
    )

    if summary["analyzer_present_count"] > summary["mapper_present_count"]:
        explanations.append(
            "The tracked triplet appears in analyzer-layer candidate rows more often than it appears in mapper output, which suggests mapper-side filtering or emission gating."
        )
    ...
    explanations.append(
        "If analyzer-layer triplet evidence remains present while mapper triplet fields drop to None/empty, the decisive bottleneck is mapper rule application rather than upstream identity visibility."
    )
...
def render_markdown(report: Dict[str, Any]) -> str:
    ...
    lines: List[str] = []
    lines.append("# Mapper Triplet Rule Diagnosis")
    lines.append("")
    ...
    for row in rows:
        lines.append(
            "| " + " | ".join([
                ...
                safe_cell(row.get("comparison_edge_candidate_strength_latest")),
                safe_cell(row.get("comparison_symbol_has_tracked_horizon")),
                safe_cell(row.get("comparison_strategy_has_tracked_horizon")),
                safe_cell(row.get("analyzer_triplet_present")),
                safe_cell(row.get("analyzer_selected_candidate_strength")),
                safe_cell(row.get("analyzer_selected_stability_label")),
                safe_cell(row.get("analyzer_selected_visible_horizons")),
                safe_cell(row.get("mapper_triplet_present")),
                safe_cell(row.get("mapper_triplet_candidate_strength")),
                safe_cell(row.get("mapper_triplet_stability_label")),
                safe_cell(row.get("mapper_triplet_visible_horizons")),
