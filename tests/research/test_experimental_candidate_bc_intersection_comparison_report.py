...
def _extract_shared_overall_metrics(candidate_summary: dict[str, Any], horizon: str) -> dict[str, float | None]:
    median_payload = _safe_dict(
        _safe_dict(candidate_summary.get("median_future_return_by_horizon")).get(horizon)
    )
    positive_payload = _safe_dict(
        _safe_dict(candidate_summary.get("positive_rate_by_horizon")).get(horizon)
    )
    negative_payload = _safe_dict(
        _safe_dict(candidate_summary.get("negative_rate_by_horizon")).get(horizon)
    )

    positive_ratio = (
        _safe_float(positive_payload.get("ratio"))
        if int(positive_payload.get("numeric_row_count", 0)) > 0
        else None
    )
    negative_ratio = (
        _safe_float(negative_payload.get("ratio"))
        if int(negative_payload.get("numeric_row_count", 0)) > 0
        else None
    )

    return {
        "median_future_return_pct": _safe_float(median_payload.get("overall")),
        "positive_rate_pct": positive_ratio,
        "negative_rate_pct": negative_ratio,
    }
    ...
    for group_name in sorted(grouped):
        ...
        for horizon in TARGET_HORIZONS:
            ...
            by_horizon[horizon] = {
                ...
                "positive_rate_pct": _safe_ratio(positive_count, numeric_row_count) if numeric_row_count else None,
                "negative_rate_pct": _safe_ratio(negative_count, numeric_row_count) if numeric_row_count else None,
        ...
        for group_name in sorted(set(baseline) | set(experiment)):
            ...
            row = {
                ...
                "negative_rate_pct": _compare_numeric(
                    _safe_float(baseline_h.get("negative_rate_pct")),
                    _safe_float(experiment_h.get("negative_rate_pct")),
                ),
...
def build_experimental_candidate_bc_intersection_comparison_report(
    ...
    *,
    ...
    shared_metric_comparison: dict[str, Any] = {}
    for horizon in TARGET_HORIZONS:
        baseline_metrics = _extract_shared_overall_metrics(baseline_shared_summary, horizon)
        experiment_metrics = _extract_shared_overall_metrics(experiment_shared_summary, horizon)
        flat_ratio_baseline = _safe_float(
            _safe_dict(_safe_dict(_safe_dict(shared_label_distribution.get(horizon)).get("flat")).get("ratio")).get("baseline")
        )
        flat_ratio_experiment = _safe_float(
            _safe_dict(_safe_dict(_safe_dict(shared_label_distribution.get(horizon)).get("flat")).get("ratio")).get("experiment")
        )

        shared_metric_comparison[horizon] = {
            "median_future_return_pct": _compare_numeric(
                baseline_metrics["median_future_return_pct"],
                experiment_metrics["median_future_return_pct"],
            ),
            "positive_rate_pct": _compare_numeric(
                baseline_metrics["positive_rate_pct"],
                experiment_metrics["positive_rate_pct"],
            ),
            "negative_rate_pct": _compare_numeric(
                baseline_metrics["negative_rate_pct"],
                experiment_metrics["negative_rate_pct"],
            ),
            "flat_rate_pct": _compare_numeric(
                flat_ratio_baseline,
                flat_ratio_experiment,
            ),
        }
        ...
        for horizon in TARGET_HORIZONS:
            ...
            for row in rows:
                ...
                lines.append(
                    ...
                    f"negative_rate_delta={_format_pct(_safe_float(_safe_dict(row_payload.get('negative_rate_pct')).get('delta')))}, "
